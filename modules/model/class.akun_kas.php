<?php
	...
	class akun_kas extends koneksi {
	
		function get_data() {
			$query = "SELECT `kode`, `nama_akun` FROM `akun_kas` WHERE `hapus` = '0';";
		...
		function tambah_baru($kode, $nama) {
			$kode = $this->clearText($kode);
			$nama = $this->clearText($nama);
			
			if ($result = $this->runQuery("INSERT INTO `akun_kas` VALUES('$kode', '$nama', '0');")) {
				return TRUE;
			} else {
				return FALSE;
			}
		...
		function ubah_data($kode, $nama) {
			$kode = $this->clearText($kode);
			$nama = $this->clearText($nama);
			
			if ($result = $this->runQuery("UPDATE `akun_kas` SET `nama_akun` = '$nama' WHERE `kode` = '$kode';")) {
				return TRUE;
			} else {
				return FALSE;
			}
		...
		function hapus_data($kode) {
			...
			if ($result = $this->runQuery("UPDATE `akun_kas` SET `hapus` = '1' WHERE `kode` = '$kode';")) {
				return TRUE;
			} else {
				return FALSE;
			}
