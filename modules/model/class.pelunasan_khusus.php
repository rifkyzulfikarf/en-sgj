<?php
	//eval(base64_decode(""));

	include("class.bank_khusus.php");
	
	class pelunasan_khusus extends koneksi {
		
		function autocode_pelunasan($tgl) {
			$kode = "";
			$query = "SELECT COUNT(`id`) FROM `khusus_pelunasan` WHERE `tgl` = '$tgl';";
			if ($result = $this->runQuery($query)) {
				$rs = $result->fetch_array();
				
				switch (strlen($rs[0] + 1)) {
					case 1:
						$kode = "KPP".date("ymd", strtotime($tgl))."000".($rs[0] + 1);
						break;
					case 2:
						$kode = "KPP".date("ymd", strtotime($tgl))."00".($rs[0] + 1);
						break;
					case 3:
						$kode = "KPP".date("ymd", strtotime($tgl))."0".($rs[0] + 1);
						break;
					case 4:
						$kode = "KPP".date("ymd", strtotime($tgl)).($rs[0] + 1);
						break;
				}
			}
			return $kode;
		}
		
		function get_penjualan_belum_lunas($tglAwal, $tglAkhir) {
			$tglAwal = $this->clearText($tglAwal);
			$tglAkhir = $this->clearText($tglAkhir);
			
			$query = "SELECT `khusus_penjualan`.*, `khusus_barang`.`nama` AS `nama_barang`, `konsumen`.`nama` AS `nama_konsumen`, 
			`sales`.`nama` AS `nama_sales` FROM `khusus_penjualan` INNER JOIN `khusus_barang` ON (`khusus_penjualan`.`id_barang` = `khusus_barang`.`id`) 
			INNER JOIN `konsumen` ON (`khusus_penjualan`.`id_konsumen` = `konsumen`.`id`) 
			INNER JOIN `karyawan` AS `sales` ON (`khusus_penjualan`.`id_sales` = `sales`.`id`) 
			WHERE `khusus_penjualan`.`jenis` = '4' AND `khusus_penjualan`.`total_bayar` < `khusus_penjualan`.`total_jual` 
			AND `khusus_penjualan`.`tgl_tempo` BETWEEN '$tglAwal' AND '$tglAkhir';";
			
			if ($list = $this->runQuery($query)) {
				if ($list->num_rows > 0) {
					return $list;
				} else {
					return FALSE;
				}
			} else {
				return FALSE;
			}
		}
		
		function get_penjualan_belum_lunas_by_konsumen($idKonsumen) {
			$idKonsumen = $this->clearText($idKonsumen);
			
			$query = "SELECT `khusus_penjualan`.*, `khusus_barang`.`nama` AS `nama_barang`, `konsumen`.`nama` AS `nama_konsumen`, 
			`sales`.`nama` AS `nama_sales` FROM `khusus_penjualan` INNER JOIN `khusus_barang` ON (`khusus_penjualan`.`id_barang` = `khusus_barang`.`id`) 
			INNER JOIN `konsumen` ON (`khusus_penjualan`.`id_konsumen` = `konsumen`.`id`) 
			INNER JOIN `karyawan` AS `sales` ON (`khusus_penjualan`.`id_sales` = `sales`.`id`) 
			WHERE `khusus_penjualan`.`jenis` = '4' AND `khusus_penjualan`.`total_bayar` < `khusus_penjualan`.`total_jual` 
			AND `konsumen`.`id` = '$idKonsumen';";
			
			if ($list = $this->runQuery($query)) {
				if ($list->num_rows > 0) {
					return $list;
				} else {
					return FALSE;
				}
			} else {
				return FALSE;
			}
		}
		
		function get_bg_belum_ambil($tglAwal, $tglAkhir) {
			$tglAwal = $this->clearText($tglAwal);
			$tglAkhir = $this->clearText($tglAkhir);
			
			$query = "SELECT `khusus_pelunasan`.*, `konsumen`.`nama` FROM `khusus_pelunasan` 
					INNER JOIN `khusus_penjualan` ON (`khusus_pelunasan`.`id_penjualan` = `khusus_penjualan`.`id`) 
					INNER JOIN `konsumen` ON (`khusus_penjualan`.`id_konsumen` = `konsumen`.`id`) WHERE `khusus_pelunasan`.`jenis` = '4' AND 
					`khusus_pelunasan`.`ambil_bg` = '0' AND `khusus_pelunasan`.`tgl_bg` BETWEEN '$tglAwal' AND '$tglAkhir';";
			
			if ($list = $this->runQuery($query)) {
				if ($list->num_rows > 0) {
					return $list;
				} else {
					return FALSE;
				}
			} else {
				return FALSE;
			}
		}
		
		function bayar_pelunasan($idPenjualan, $tgl, $totalBayar, $jenis, $tglBg, $idBank, $noBukti, $idKaryawan){
			$idPenjualan = $this->clearText($idPenjualan);
			$tgl = $this->clearText($tgl);
			$totalBayar = $this->clearText($totalBayar);
			$jenis = $this->clearText($jenis);
			$tglBg = $this->clearText($tglBg);
			$idBank = $this->clearText($idBank);
			$noBukti = $this->clearText($noBukti);
			$idKaryawan = $this->clearText($idKaryawan);
			
			$id = $this->autocode_pelunasan($tgl);
			$ambilBg = "0";
			
			if ($jenis != "4") {
				$tglBg = "0000-00-00";
				$ambilBg = "1";
			}
			
			$query = "INSERT INTO `khusus_pelunasan` VALUES('$id', '$idPenjualan', '$tgl', '$totalBayar', '$jenis', '$tglBg', 
			'$ambilBg', '$idBank', '$noBukti', '$idKaryawan');";
			$query .= "UPDATE `khusus_penjualan` SET `total_bayar` = `total_bayar` + '$totalBayar' WHERE `id` = '$idPenjualan';";
			
			if ($result = $this->runMultipleQueries($query)) {
				if ($jenis == "2" || $jenis == "3") {
					$bank = new bank_khusus();
					$hasilBank = $bank->transaksi_setor($idBank, $noBukti, $tgl, "Piutang ".$id, $totalBayar, $idKaryawan);
				} else {
					$hasilBank = TRUE;
				}
				
				if ($hasilBank) {
					return TRUE;
				} else {
					return FALSE;
				}
			} else {
				return FALSE;
			}
		}
		
		function ambil_bg($idPelunasan, $tglBg, $totalBayar, $idBank, $bukti, $idKaryawan){
			$idPelunasan = $this->clearText($idPelunasan);
			$tglBg = $this->clearText($tglBg);
			$totalBayar = $this->clearText($totalBayar);
			$idBank = $this->clearText($idBank);
			$bukti = $this->clearText($bukti);
			$idKaryawan = $this->clearText($idKaryawan);
			
			$query = "UPDATE `khusus_pelunasan` SET `ambil_bg` = '1' WHERE `id` = '$idPelunasan';";
			
			if ($result = $this->runQuery($query)) {
				$bank = new bank_khusus();
				$hasilBank = $bank->transaksi_setor($idBank, $bukti, $tglBg, "Pencairan BG ".$bukti, $totalBayar, $idKaryawan);
				
				if ($hasilBank) {
					return TRUE;
				} else {
					return FALSE;
				}
			} else {
				return FALSE;
			}
			
		}
		
	}
	
?>