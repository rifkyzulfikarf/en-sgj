<?php
	...
	class penjualan_khusus extends koneksi {
		...
		function autocode_penjualan($tgl) {
			...
			$query = "SELECT MAX(`id`) FROM `khusus_penjualan` WHERE `tgl` = '$tgl' AND `id` LIKE '$prefix%';";
		...
		$totalBayar, $jenis, $tglTempo, $idBank, $noBukti, $idSales, $idKaryawan, $nota) {
			...
			$query = "INSERT INTO `khusus_penjualan` VALUES('$id', '$tgl', '$idKonsumen', '$idBarang', '$jmlTabung', '$hargaJual', 
						'$het', '$totalJual', '$totalHet', '$totalBayar', '$jenis', '$tglTempo', '$idBank', '$noBukti', 
						'$idSales', '$idKaryawan', '$nota');";
			$query .= "INSERT INTO `khusus_penjualan_acc_gudang`(`id_penjualan`) VALUES('$id');";
		...
		function acc_gudang($idPenjualan, $idBarang, $jml, $acc, $idGudang) {
			...
			$query = "UPDATE `khusus_penjualan_acc_gudang` SET `acc_gudang` = '$acc', `id_gudang` = '$idGudang' WHERE `id_penjualan` = '$idPenjualan';";
			if ($acc == "1") {
				$query .= "UPDATE `khusus_barang` SET `stok_isi` = `stok_isi` - $jml, `stok_kosong` = `stok_kosong` + $jml WHERE `id` = '$idBarang';";
			}
