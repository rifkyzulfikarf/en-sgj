			...
			<div class="modal-body">
				<form id="frm-harga" action="#" method="POST" role="form">
					...
					<div class="form-group">
						<input type="text" class="form-control" id="txt-50kg-khusus" name="txt-50kg-khusus" placeholder="Harga Jual 50kg Khusus">
					</div>
				</form>
			</div>
...
$(document).ready(function(){
	...
	var tabelharga = $('#tabel-harga').dataTable({
		...
		"fnServerParams": function ( aoData ) {
            aoData.push({"name": "aksi", "value": "<?php echo e_url('modules/controller/khusus/harga-jual.php'); ?>"});
	...
	$('#tabel-harga').on('click', '#btn-ubah-data', function(ev){
		...
		$('#txt-50kg-khusus').val($(this).data('50kgkhusus'));
	});
	
	
	$('#btn-simpan-data').click(function(ev){
		ev.preventDefault();
		var post_data = "aksi=" + "<?php echo e_url('modules/controller/khusus/harga-jual.php'); ?>" + "&" +$('#frm-harga').serialize();
