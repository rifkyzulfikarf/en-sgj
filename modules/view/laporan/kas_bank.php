				...
				<div class="panel-body">
					...
					?>
						...
						<div id="print-section">
							<div class="text-center"><h3>Laporan Kas Bank <?php echo ($_POST['bank'] == "1")?"PT. Energas Nusantara":"PT. Sumber Gasindo Jaya"; ?></h3></div><hr>
							...
							<table class="table table-hover table-striped table-mod">
								...
								<tbody>
									<?php
									$query = "SELECT `kas_bank`.`tgl`, `kas_bank`.`no_bukti`, `kas_bank`.`keterangan`, `kas_bank`.`jenis`, 
									`kas_bank`.`setor`, `kas_bank`.`tarik`, `kas_bank`.`bea_admin`, `kas_bank`.`saldo` FROM `kas_bank` 
									INNER JOIN `bank` ON (`kas_bank`.`id_bank` = `bank`.`id`)  
									WHERE `kas_bank`.`id_bank` LIKE '".$_POST['bank']."' 
									AND `kas_bank`.`tgl` BETWEEN '".$_POST['tgl-awal']."' AND '".$_POST['tgl-akhir']."';";
		...
		<div class="modal-dialog">
			<div class="modal-content">
				...
				<div class="modal-body">
					<input type="hidden" id="no_spa" name="no_spa" value="<?php echo e_url('modules/view/laporan/kas_bank.php'); ?>">
					<div class="form-group">
						<select class="form-control" id="bank" name="bank">
							<?php
								if ($query = $data->runQuery("SELECT `bank`.`id`, `bank`.`nama` FROM `bank`;")) {
